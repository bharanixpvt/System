...
export function TrainingModal({ pathName, onClose }: { pathName?: TrainingPathName; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[70] bg-black/75 backdrop-blur-sm flex items-end sm:items-center justify-center p-4"><section className="glass-card w-full max-w-md p-5 border-[#CBD5E1]/25">
    <div className="flex justify-between items-center"><div><p className="system-text text-[#CBD5E1]">QUEST TRAINING</p><h2 className="text-lg font-bold">{selected.displayName}</h2></div><button onClick={onClose}><X className="text-white/50" /></button></div>
