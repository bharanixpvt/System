...
export function BossDungeonAlert() {
  const { activeBossDungeon, dismissBossDungeon, navigateTo, summonBossDungeon } = useGameStore();
  const [, tick] = useState(0);
  useEffect(() => { const id = window.setInterval(() => tick(n => n + 1), 1000); const invite = window.setTimeout(() => { if (Math.random() > .35) summonBossDungeon(); }, 45000); return () => { clearInterval(id); clearTimeout(invite); }; }, [summonBossDungeon]);
  if (!activeBossDungeon) return null;
